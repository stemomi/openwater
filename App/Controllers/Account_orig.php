...
class Account extends \Core\Controller
{
    ...
    public function Register()
    {
        ...
        $DataNascitaMassima = DateTime::createFromFormat('Y-m-d', date('Y-m-d'))->modify('-11 year')->format('Y-m-d');

        $ArrayTemplateAction= [
            'ProvinceItaliane' => $ProvinceItaliane,
            'Squadre' => $Squadre,
            'DataNascitaMassima' => $DataNascitaMassima
        ];
        $ArrayTemplateGate= ['URL_TO_PUBLIC_FOLDER' => URL_TO_PUBLIC_FOLDER];
        $ArrayTemplateCombined= $ArrayTemplateGate + $ArrayTemplateAction;
    ...
    public function RegisterConfirm()
    {
        if (isset($_POST['Email']) && isset($_POST['Password']))
        {
            ...
            $ControllaCambioUtente= $AccountClass->ControllaEmailEsistente($Email);
    ...
    public function Iscrizioni()
    {
        ...
        $CheckAcquistoMaglietta1 = $GareClass->checkUtenteAcquistoMaglietta($Utente->ID, 1);
        $CheckAcquistoMaglietta2 = $GareClass->checkUtenteAcquistoMaglietta($Utente->ID, 2);
        $CheckAcquistoMaglietta3 = $GareClass->checkUtenteAcquistoMaglietta($Utente->ID, 3);
    ...
    public function CaricaBonifico()
    {
        ...
        if ($Impostazioni_AbilitaBonifico < 1)
        ...
        if ( isset($_FILES['FileBonifico']) && isset($_POST['DataBonifico']) && isset($_POST['ImportoBonifico']) )
        {
            ...
            if ( $_FILES["FileBonifico"]["name"] != '' )
            {
                ...
                if (move_uploaded_file($_FILES["FileBonifico"]["tmp_name"], PUBLIC_PATH . $path_bonifico))
                {
                    ...
                    $messaggio .= ' <br />
                                    <p>Per Bonifico: Conto intestato ad ASD I GLACIALI, IBAN  IT60X0521610800000000023275.
    ...
    public function EditProfile()
    {
        ...
        $data_nascita_massima = DateTime::createFromFormat('Y-m-d', date('Y-m-d'))->modify('-11 year')->format('Y-m-d');
        ...
        [
            ...
            'ProvinceItaliane' => $ProvinceItaliane,
            'Squadre' => $Squadre,
            'data_nascita_massima' => $data_nascita_massima
        ];
    ...
    public function ForgotPasswordConfirm()
    {
        ...
        if (isset($_POST['Email']))
        {
            ...
            if ($Utente)
            {
                ...
                <p>Ciao ' . $Utente->nome . ', per impostare una nuova password, clicca il link seguente:</p>
                <p>' . URL_TO_PUBLIC_FOLDER_NO_SLASHES . 'Account/resetPassword/' . $codice . '</p>
